...
|     4       information and downloads can still be accessed at the         |
...
class QuickSkin {
   ...
   * @var bool
   */
  public $reuse_code       = false; //overwritten in constructor!
   ...
   * @var string
   */
  public $temp_dir         = 'set_in_constructor';
   ...
   * @var string
   */
  public $cache_dir         = 'set_in_constructor';
   ...
   * @var string
   */
  public $extensions_dir    = 'willBeOverWritten';
   ...
   * @var string
   */
  public $extension_prefix  = 'TemplateExtensions::';//WOUTER: doing this all as static calls to templateextensions class formerly: 'qx_';
  ...
  public $left_translate_delimiter = '%%';
  public $right_translate_delimiter = '%%';
  ...
  public $data = array(); /* Template content array */
  public $keys_to_assign_raw = array(); /* Will not get htmlentities */
  ...
  function __construct( $template_filename = '' ) {
    //locally we always recompile, online we don't
    $this->reuse_code = false;//!Debug::debuggingIsEnabled(); //!Util::isLocalEnvironment(); // WOUTER THIS WAS GOING WRONG ONLIEN IT SEMS

    $this->temp_dir = Configuration::get('locations', 'quickskin_compiledpages');

    $this->cache_dir = Configuration::get('locations', 'quickskin_compiledpages');
    ...
    if (preg_match_all('/'.$this->left_delimiter.'([a-zA-Z0-9_]+):([^}]*)'.$this->right_delimiter.'/', $contents, $var)) {
      foreach ($var[2] as $cnt => $tag) {
        ...
        //if (!isset($this->extension_tagged[$extension])) {
            ...
            //WOUTER: changing this to put all extensions just in one class (we have autoloader anyway)
            //$header .= 'include_once "'.$this->extensions_dir."/".$this->extension_prefix."$extension.php\";\n";
            //instead of a 'prefix' we use class name:
            //$this->extension_prefix = "TemplateExtensions::";
   ...
   */
  function assign ( $name, $value = '', $raw = false ) {
    if (is_array($name)) {
      foreach ($name as $k => $v) {
        $this->data[$k]  =  $v;
        
        //raw? (i.e. no htmlentities)
        if ($raw)
        {
            $this->keys_to_assign_raw[] = $k;
        }
      }
    } else {
  ...
  function setOutputLanguage($languageCode)
  {
      $this->sitelanguage = $languageCode;
  }
  ...
  function output ( $_top = '' ) 
  {
    ...
    //@WOUTER SECOND CHANGE: JUST MAKING COMPILED FILE SAME FILENAME AS ORIGINAL TPL
    //before it was this:
    //$cpl_file_name = preg_replace('/[:\/.\\\\]/', '_', $this->tpl_file . '?' . $queryString);
    $cpl_file_name = str_replace('/','_',$this->tpl_file . ($this->sitelanguage ? '.' . $this->sitelanguage : ''));
    //echo $this->sitelanguage;
    if (strlen($cpl_file_name) > 0) 
    {
        
         //@WOUTER changed this: adding template file in compiled filename
          //previous: $cpl_file_name = 'qs_' . md5($cpl_file_name);
          //this was after first change:
          //$cpl_file_name = 'qs_' . str_replace(array('/', '?', '.', 'html'), '_', $this->tpl_file) .  '_' . md5($cpl_file_name);

          $this->cpl_file  =  $this->temp_dir . 'compiled.' . $cpl_file_name . '.php';
          ...
          if ($this->reuse_code) 
          {
                if (is_file($this->cpl_file)) 
                {
                    if ($this->mtime($this->cpl_file) > $this->mtime($this->template_dir . $this->tpl_file)) 
                      {
                        //die('here, tpl file:' . $this->cpl_file);
                        $compile_template  =  false;
                      }
                      else
                      {
                          //echo "Template file changed since compile file! Recompiling!";
                      }
                }
                else
                {
                    //echo "No compiled file found! Compiling";
                }
          }

         //echo "Template needs to be compiled? ->" . ($compile_template ? 'yes' : 'no');

          if ($compile_template) {
              //echo 'compiling!';
            $this->parser = new QuickSkinParser();
            $this->parser->keys_to_assign_raw      = $this->keys_to_assign_raw;
            ...
            $this->parser->left_translate_delimiter  = $this->left_translate_delimiter;
            $this->parser->right_translate_delimiter  = $this->right_translate_delimiter;
            ...
            $this->parser->sitelanguage   = $this->sitelanguage;
            if (!$this->parser->compile($this->cpl_file,$data,$this->supp_templates,$this->extensions_dir)) {
             Debug::error('QuickSkin Parser Error: ' . $this->parser->error);
            }
          }

          //echo 'her' . $this->cpl_file . 'file?';
...
class QuickSkinParser {
  ...
  public $keys_to_assign_raw = array();   /* keys for aqssignes that shouldnt be html entities @wouter */
  ...
  public $left_translate_delimiter;      /* Default Left translate delimiter */
  public $right_translate_delimiter;      /* Default Right translate delimiter */
  ...
  public $sitelanguage = false;    /* language to compile this template in */
  ...
  */
  ...
  function wrapPageInFile($page, $file)
  {
	   $p = new QuickSkin($file);

		$t = $p->result(); 
		$page = preg_replace('/(<!-- WRAPPER template < (.*) -->)/i', '', $page);
		$page = str_replace('<!-- WRAPPEE [content] -->', $page, trim($t));
		return $page;
  }
  ...
  function compile( $compiled_template_filename = '', $data='', $supp_templates='', $extensions_dir='' ) {
      ...
      //@WOUTER removing this, it escaped the : in my faux prefix templateextensions::
    ...
    if (empty($this->template)) {
      die('empty template! In production, whitepage error would come here?');
      return;
    }
	...
	/** BEGIN TINKERTANKS CUSTOM LOGIC WRAPPER STUFF **/
	//conditional wrapping templates <!-- WRAPPER template < wrapper.html IF nnnn --> 
	if (preg_match_all('/<!-- WRAPPER template < (.*) IF (.*) -->/', $page, $var)) {
	   $file = $var[1][0];
	   $tag = $var[2][0] . '';
	   
       if (isset($data[$tag]) && $data[$tag])
		{
		   $page = $this->wrapPageInFile($page, $file);
		}
    }

	//wrapping templates <!-- WRAPPER template < wrapper.html -->
	elseif (preg_match_all('/<!-- WRAPPER template < (.*) -->/', $page, $var)) {
	   $file = $var[1][0];
	   $page = $this->wrapPageInFile($page, $file);
    }

	//including css files
	if (preg_match_all('/<!-- TT css="([A-z]+).css" -->/i', $page, $var)) {
		for ($i = 0; $i < sizeof($var[1]); $i++)
		{
			$file = $var[1][$i];
			$page = preg_replace('/(<!-- TT css="' . $file . '.css" -->)/i', '', $page);
			$this->cssFiles[] = $file;
		}
        }
                
	//serving minified js files using php minify script
        //http://code.google.com/p/minify/source/browse/trunk/min/README.txt
        /*
        echo 'starting grouping+minifying js';
        if (preg_match_all('/\/js\/([0-9a-z.]+\.js)/', $page, $var)) {
            $minifiedPath = 'http://localhost/min/?f=';
            $basefolder = 'whatshop/js/';
	    for ($i = 0; $i < sizeof($var[1]); $i++)
		{
			$file = $var[1][$i];
                        //echo $file . '<br>';
                        $minifiedFiles[] = $basefolder . $file;
			$page = preg_replace('/(<!-- TT css="' . $file . '.css" -->)/i', '', $page);
			$this->cssFiles[] = $file;
		}
                
                $minifiedPath = $minifiedPath . implode(',', $minifiedFiles);
                $asScriptInclude = '<script type="text/javascript" src="' . $minifiedPath . '"></script>';
                $page = str_replace("</head>", "$asScriptInclude\n</head>", $page);
        }
        die('here <a href="' . $minifiedPath . '" target="_blank">Link</a>');
	*/
        
	/** END TINKERTANKS CUSTOM LOGIC **/
    ...
    if (preg_match_all('/<!-- (ELSE)?IF {([a-zA-Z0-9_]+):([^}]*)}\s*([!=<>]+)\s*(["]?[^"]*["]?) -->/', $page, $var)) {
      foreach ($var[2] as $cnt => $tag) {
        ...
        if (!isset($this->extension_tagged[$extension])) {
            ...
            //WOUTER doing this with static calls to functions in extensions class
            //$header .= 'include_once  "'.$this->extensions_dir."/".$this->extension_prefix."$extension.php\";\n";
          $this->extension_tagged[$extension] = true;
        }
    ...
    if (preg_match_all('/'.$this->left_delimiter.'([a-zA-Z0-9_. >]+)'.$this->right_delimiter.'/', $page, $var)) {
      foreach ($var[1] as $fulltag) 
      {
        ...
        //WOUTER TINKERTANKS CHANGE: html encode vars!
        if (in_array($skalar, $this->keys_to_assign_raw))
        {
            $output = "\$$block"."['$skalar'];";            
        }
        else
        {
            $output = "str_replace('<script','&lt;script',\$$block"."['$skalar']);";
        }
    ...
    //WOUTER: originlly was like this, but we want to translate WHATEVER is between the delimiters
    //if (preg_match_all('/'.$this->left_translate_delimiter.'([a-zA-Z0-9_. ]+)'.$this->right_translate_delimiter.'/', $page, $var)) 
    
    if (preg_match_all('/'.$this->left_translate_delimiter.'(.*)'.$this->right_translate_delimiter.'/', $page, $var)) 
    {
      foreach ($var[1] as $tag) 
      {
        //echo "found translate tag: $tag<br />";
        
        //@WOUTER if we know the language we compile the translation straight into the template
        if ($this->sitelanguage)
        {
            $code = Translate::compileTranslatedText($tag, $this->sitelanguage);
        }
        else
        {
            $code  =  $tag; /*"<?php echo Translate::gettext('$tag');?>"; //NOT GOOD TO DO ONJ THE FLY WE COMPILE THIS  */
        }
        
        $page  =  str_replace($this->left_translate_delimiter.$tag.$this->right_translate_delimiter,  $code,  $page);
      }
    }

   /* Include Extensions of the form {{}}}!!!! WOUTER: made this using DOUBLE delimiters */
    if (preg_match_all('/'.$this->left_delimiter.$this->left_delimiter.'([a-zA-Z0-9_]+):([^}]*)'.$this->right_delimiter.$this->right_delimiter.'/', $page, $var)) {
      foreach ($var[2] as $cnt => $tag) {
        /* Determine Command (echo / $obj[n]=) */
        ...
        if (!isset($this->extension_tagged[$extension])) {
            ...
            //WOUTER: doing extensions all as methods of one central 'extensions' class 
          //$header .= 'include_once "'.$this->extensions_dir."/".$this->extension_prefix."$extension.php\";\n";
          $this->extension_tagged[$extension]  =  true;
        }
        if (!strlen($tag)) {
	...
	if (preg_match_all('/<!-- TT \[css\] -->/', $page, $var) && ($this->tpl_file !== 'main.wrapper.html'))
	{
		$files = '';
		foreach ($this->cssFiles as $file)
		{
			$files .= "<link href=\"css/$file.css\" rel=\"stylesheet\" type=\"text/css\" media=\"screen\" />\n";
		}
		
		$page = str_replace('<!-- TT [css] -->', $files, $page);
	}
    /* Add Include Header */
    ...
    if (strlen($compiled_template_filename)) {
      if ($hd  =  fopen($compiled_template_filename,  'w')) {
        ...
        Debug::error ('Could not write compiled file.');
        return false;
      }
    } else {
      return $page;
  ...
  function worx_tpl_swap($tpldata, $data, $supp_templates) { /* do the substitution of the sub templates here */
    ...
    if ( isset($data['tpl_img']) && $data['tpl_img'] != '' && $data['url_img'] != '' ) {
    ...
    if ( isset($data['tpl_js']) && $data['tpl_js'] != '' && $data['url_js'] != '' ) {
    ...
    if ( isset($data['tpl_css']) && $data['tpl_css'] != '' && $data['url_css'] != '' ) {
